...
// ─────────────────────────────────────────────────────────────────────────────
// POST /api/driver/nfc-scan
// Driver taps a passenger's NFC card at the bus reader.
// Mirrors scan-qr: looks up the card owner, deducts fare within a transaction.
// Redis cooldown prevents the same card+trip from being processed twice in 30 s.
// Body: { uid: "AB:CD:EF:01", trip_id?: number }
// ─────────────────────────────────────────────────────────────────────────────
export const scanNfc = async (req, res) => {
  const { uid, trip_id } = req.body;
  if (!uid) return res.status(400).json({ valid: false, message: "uid required" });

  const cleanUid = String(uid).trim().toUpperCase();

  // Redis cooldown: 30 s per (uid, trip) — prevents accidental double-tap billing
  const cooldownKey = `nfc:cooldown:${cleanUid}:${trip_id ?? "notripid"}`;
  try {
    const redis  = getRedis();
    const result = await redis.set(cooldownKey, "1", "EX", 30, "NX");
    if (result !== "OK") {
      return res.status(429).json({ valid: false, message: "Card already scanned — please wait 30 seconds before tapping again." });
    }
  } catch { /* Redis unavailable — proceed without cooldown */ }

  const pool = await poolPromise;

  // Look up the passenger who owns this NFC card
  const cardResult = await pool.request()
    .input("uid", sql.NVarChar(64), cleanUid)
    .query(`
      SELECT TOP 1 user_id
      FROM   nfc_cards
      WHERE  card_uid = @uid AND status = 'active'
    `);

  if (!cardResult.recordset[0]) {
    return res.status(404).json({ valid: false, message: "NFC card not recognised or not linked to any account." });
  }

  const userId = cardResult.recordset[0].user_id;

  // Run the same fare-deduction transaction as scan-qr
  const tx = pool.transaction();
  try {
    await tx.begin();

    // Get latest confirmed ticket for this passenger
    const ticketRes = await tx.request()
      .input("uid", sql.Int, userId)
      .query(`
        SELECT TOP 1 t.ticket_id, t.trip_id, t.seat_number, t.amount,
                     u.full_name, u.email
        FROM   tickets t
        JOIN   users   u ON u.user_id = t.user_id
        WHERE  t.user_id = @uid AND t.status = 'confirmed'
        ORDER  BY t.ticket_id DESC
      `);
    ...
    if (ticket?.amount) {
      ...
      if (wallet && !wallet.is_frozen && parseFloat(wallet.balance) >= fare) {
        ...
        await tx.request()
          .input("uid",  sql.Int,           userId)
          .input("fare", sql.Decimal(10, 2), fare)
          .input("tid",  sql.Int,           ticket.ticket_id)
          .query(`
            INSERT INTO wallet_transactions (user_id, type, amount, description, created_at)
            VALUES (@uid, 'debit', @fare, CONCAT('NFC boarding — Ticket #', @tid), GETUTCDATE())
          `);

        await tx.request()
          .input("tid", sql.Int, ticket.ticket_id)
          .query(`UPDATE tickets SET status = 'boarded' WHERE ticket_id = @tid`);
    ...
    await tx.rollback().catch(() => {});
    console.error("[nfc-scan]", err);
    return res.status(500).json({ valid: false, message: "Scan processing failed." });
  }
};
