        ...
        <div style={{ ...cardStyle, marginBottom: 20 }}>
          ...
          <div style={{ padding: "16px 28px", display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, borderBottom: `1px solid ${C.border}` }}>
            {[
              ...
              { label: "Total Processed", value: fmt(report.total_amount, currency, exchangeRate) },
              { label: "Cash Collected",  value: fmt(report.cash_amount, currency, exchangeRate) },
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12 }}>
                    ...
                    return (
                      <tr key={tx.id || i} style={{ borderBottom: `1px solid ${C.border}`, background: i % 2 === 0 ? "#fff" : "#FAFBFC" }}>
                        ...
                        <td style={{ padding: "9px 14px", fontWeight: 800, color: C.primary, whiteSpace: "nowrap" }}>
                          +{fmt(tx.amount, currency, exchangeRate)}
                        </td>
                        <td style={{ padding: "9px 14px" }}>
                ...
                <tfoot>
                  <tr style={{ background: "#F8FAFC", borderTop: `2px solid ${C.border}` }}>
                    ...
                    <td style={{ padding: "10px 14px", fontWeight: 900, color: C.primary, fontSize: 14 }}>{fmt(report.total_amount, currency, exchangeRate)}</td>
                    <td colSpan={3} />
                  </tr>
                </tfoot>
              </table>
                ...
                {Object.entries(byMethod).map(([method, { count, total }]) => {
                  ...
                  return (
                    <div key={method} style={{ background: ms.bg, borderRadius: 10, padding: "12px 14px", border: `1px solid ${ms.color}22` }}>
                      ...
                      <div style={{ fontSize: 18, fontWeight: 800, color: "#0F172A" }}>{fmt(total, currency, exchangeRate)}</div>
        ...
        <div style={{ ...cardStyle, marginBottom: 20 }}>
          ...
          <div style={{ padding: "0 20px 20px" }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 0, border: `1px solid ${C.border}`, borderRadius: 10, overflow: "hidden", marginTop: 16 }}>
              {[
                { label: "Opening Cash",            value: fmt(report.opening_cash, currency, exchangeRate),    note: "Cash on hand at shift start"              },
                { label: "Cash Top-Ups Collected",  value: fmt(report.cash_amount, currency, exchangeRate),     note: `${cashTxs.length} Cash transactions`       },
                { label: "Expected Closing Cash",   value: fmt(expectedClosing, currency, exchangeRate),        note: "Opening + Cash collected", bold: true, accent: C.primary },
                ...(report.closing_cash != null ? [
                  { label: "Reported Closing Cash", value: fmt(report.closing_cash, currency, exchangeRate),    note: "As counted by staff"                      },
                  {
                    label:     "Discrepancy",
                    value:     (() => { const d = discrepancy; return `${d > 0 ? "+" : ""}${fmt(Math.abs(d), currency, exchangeRate)}`; })(),
              ...
              <div style={{
                ...
                <div>
                  <div style={{ fontSize: 13, fontWeight: 700, color: isShort ? "#B91C1C" : "#92400E" }}>
                    {isShort ? `Cash shortage of ${fmt(Math.abs(discrepancy), currency)} detected` : `Cash overage of ${fmt(discrepancy, currency, exchangeRate)} detected`}
                  </div>
                ...
                <tbody>
                  {pendingItems.map((item, i) => (
                    <tr key={item.id} style={{ borderTop: `1px solid ${C.border}`, background: i % 2 === 0 ? "#fff" : "#FEF2F2" }}>
                      ...
                      <td style={{ padding: "8px 14px", fontWeight: 800, color: C.primary }}>{fmt(item.payload?.amount, currency, exchangeRate)}</td>
