...
import {
  ...
  Platform, Linking, Modal, Share, Dimensions, Easing, Pressable,
} from 'react-native';
...
import Svg, { Defs, LinearGradient as SvgGradient, Stop, Rect, Circle, Line } from 'react-native-svg';
...
const txTitle = (t) =>
  t.type === 'credit'
    ? 'Wallet Top-Up'
    : (t.description && !/^In-person recharge/i.test(t.description) ? t.description : 'Trip Payment');

const txSubtitle = (t) =>
  t.type === 'credit'
    ? [t.location_name, t.processed_by].filter(Boolean).join('  ·  ') || 'In-person recharge'
    ...
    >
      {box.w > 0 && box.h > 0 && (
        <Svg width={box.w} height={box.h}>
          <Defs>
            <SvgGradient id={id} x1="0" y1="0" x2={box.w} y2={box.h} gradientUnits="userSpaceOnUse">
              {colors.map((c, i) => (
...
const CardPattern = ({ w, h }) => (
  <Svg width={w} height={h} style={StyleSheet.absoluteFill} pointerEvents="none">
    <Circle cx={w - 24} cy={34} r={62}  stroke="rgba(255,255,255,0.12)" strokeWidth={1.4} fill="none" />
    <Circle cx={w - 24} cy={34} r={104} stroke="rgba(255,255,255,0.08)" strokeWidth={1.4} fill="none" />
    <Circle cx={w - 24} cy={34} r={150} stroke="rgba(255,255,255,0.05)" strokeWidth={1.4} fill="none" />
    <Circle cx={26} cy={h - 16} r={70} stroke="rgba(255,255,255,0.06)" strokeWidth={1.2} fill="none" />
    <Line x1={0} y1={h - 44} x2={w} y2={h - 96} stroke="rgba(255,255,255,0.06)" strokeWidth={1} />
    <Line x1={0} y1={h - 22} x2={w} y2={h - 74} stroke="rgba(255,255,255,0.045)" strokeWidth={1} />
  </Svg>
);
...
const Bounce = ({ children, onPress, style, scaleTo = 0.96, disabled }) => {
  const s = useRef(new Animated.Value(1)).current;
  return (
    <AnimatedPressable
      onPress={onPress}
      disabled={disabled}
      onPressIn={() => Animated.spring(s, { toValue: scaleTo, useNativeDriver: true, speed: 50, bounciness: 0 }).start()}
      onPressOut={() => Animated.spring(s, { toValue: 1, useNativeDriver: true, speed: 20, bounciness: 8 }).start()}
...
const AnimatedBalance = ({ value, style }) => {
  ...
  useEffect(() => {
    ...
    Animated.timing(anim, {
      toValue: value ?? 0, duration: 1000, easing: Easing.out(Easing.cubic), useNativeDriver: false,
    }).start();
  ...
  return (
    <Text style={style}>
      <Text>$</Text>{whole}<Text style={{ fontSize: 26 }}>.{cents}</Text>
    </Text>
  );
};
...
const WalletScreen = () => {
  ...
  const userId   = user?.user_id ?? (user?._id && /^\d+$/.test(String(user._id)) ? user._id : null);
  ...
  const last4    = String(walletId ?? userId ?? '0000').padStart(4, '0').slice(-4);
  ...
  useEffect(() => {
    Animated.stagger(85, intro.map(a =>
      Animated.spring(a, { toValue: 1, useNativeDriver: true, friction: 9, tension: 60 })
    )).start();
    Animated.loop(
      Animated.sequence([
  ...
  useEffect(() => {
    if (showCashier) {
      ...
      Animated.spring(cashierAnim, { toValue: 1, useNativeDriver: true, friction: 7, tension: 55 }).start();
      Animated.loop(
        Animated.sequence([
  ...
  useEffect(() => {
    if (selectedTx) {
      ...
      Animated.spring(receiptAnim, { toValue: 1, useNativeDriver: true, friction: 8, tension: 65 }).start();
    }
  }, [selectedTx]);

  const sect = (i) => ({
    opacity: intro[i],
    transform: [{ translateY: intro[i].interpolate({ inputRange: [0, 1], outputRange: [28, 0] }) }],
  });
  ...
  const printReceipt = async (t) => {
    ...
    const html = `
      ...
      <body>
        ...
        <div class="amt">${isCredit ? '+' : '-'}$${parseFloat(t.amount).toFixed(2)}</div>
        <div class="type">${isCredit ? 'Wallet Top-Up' : 'Trip Payment'} · Completed</div>
        <table>
          ...
          ${row('Transaction ID', t.transaction_id ? '#' + t.transaction_id : null)}
  ...
  const handleMarkerPress = (loc) => {
    ...
    if (viewMode === 'map' && hasCoords(loc)) {
      ...
      mapRef.current?.animateCamera({
        ...
        pitch: 60, heading: 30, altitude: 600, zoom: 17.5,
      }, { duration: 800 });
    }
  };
  ...
  const centerOnUser = () => {
    if (userLocation && mapRef.current) {
      mapRef.current.animateCamera({
        center: userLocation, pitch: 50, heading: 0, altitude: 900, zoom: 16.5,
      }, { duration: 700 });
    }
  };
  ...
  const onMapReady = () => {
    ...
    const center = userLocation
      ?? (first ? { latitude: Number(first.latitude), longitude: Number(first.longitude) }
  ...
  const zoomBy = async (dir) => {
    ...
    if (typeof camera.zoom === 'number') {
      camera.zoom = Math.max(3, Math.min(20, camera.zoom + (dir === 'in' ? 1 : -1)));
    } else if (typeof camera.altitude === 'number') {
      camera.altitude = dir === 'in' ? camera.altitude / 2 : camera.altitude * 2;
    }
    mapRef.current?.animateCamera(camera, { duration: 300 });
  };

  const shimmerTranslate = shimmer.interpolate({ inputRange: [0, 1], outputRange: [-CARD_W * 0.7, CARD_W * 1.1] });
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Animated.View style={[styles.cardWrap, sect(0)]}>
          <View style={styles.card}>
            {/* clipped background layer (gradient + orbs + shimmer) — kept
                separate so the card's coloured shadow isn't clipped on iOS */}
            <View style={styles.cardBg} pointerEvents="none">
              <GradientFill id="balCard" colors={['#1D4ED8', '#4F46E5', '#7C3AED']} radius={26} />
              <CardPattern w={CARD_W} h={210} />
              <View style={styles.shimmerClip}>
                <Animated.View style={[styles.shimmerBand, { transform: [{ translateX: shimmerTranslate }, { rotate: '18deg' }] }]} />
              </View>
            </View>
            ...
            <View style={styles.cardTopRow}>
              ...
              {walletFrozen ? (
                ...
                <Ionicons name="wifi" size={18} color="rgba(255,255,255,0.65)" style={{ transform: [{ rotate: '90deg' }] }} />
              )}
            </View>
            ...
            <View style={styles.balanceBlock}>
              ...
              <AnimatedBalance value={walletBalance} style={styles.balanceAmount} />
            </View>
            ...
            <View style={styles.cardBottomRow}>
              <View>
                ...
                <Text style={styles.cardFootValue} numberOfLines={1}>{userName}</Text>
              </View>
        ...
        <Animated.View style={[styles.actionWrap, sect(2)]}>
          <Bounce onPress={() => setShowCashier(true)} style={styles.cashierCta} scaleTo={0.97}>
            <View style={styles.cashierCtaBg} pointerEvents="none">
              <GradientFill id="ctaGrad" colors={['#2563EB', '#7C3AED']} radius={18} />
            </View>
            <View style={styles.cashierCtaInner}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.cashierCtaSub}>Scan your code to top up instantly</Text>
              </View>
        ...
        <View style={styles.body}>
          {/* ── Frozen banner ── */}
          {walletFrozen && (
            <Animated.View style={[styles.frozenBanner, sect(3)]}>
          ...
          <Animated.View style={sect(4)}>
            <View style={styles.sectionHeader}>
              ...
              <View style={styles.viewToggle}>
                ...
                >
                  <Ionicons name="map" size={13} color={viewMode === 'map' ? COLORS.white : COLORS.textMuted} />
                ...
                >
                  <Ionicons name="list" size={13} color={viewMode === 'list' ? COLORS.white : COLORS.textMuted} />
            ...
            <View style={styles.noticePill}>
              ...
              <Text style={styles.noticePillText}>
                Top-ups are handled in person — show your code above, or visit a station below.
              </Text>
            </View>
            ...
            {viewMode === 'map' && (
              <View style={styles.mapContainer}>
                <MapView
                  ...
                  initialCamera={{
                    center: userLocation ?? { latitude: MAP_REGION.latitude, longitude: MAP_REGION.longitude },
                    pitch: 45, heading: 0, altitude: 2400, zoom: 14.5,
                  }}
                  showsBuildings
                    ...
                    return (
                      ...
                      >
                        <View style={styles.markerWrap}>
                          ...
                          <View style={[styles.markerTip, active && { borderTopColor: COLORS.secondary }]} />
                        </View>
                      </Marker>
                    );
                  })}
                ...
                <View style={styles.mapControls}>
                  <Bounce style={[styles.mapCtrlBtn, styles.mapCtrlTop]} onPress={() => zoomBy('in')}>
                    <Ionicons name="add" size={22} color={COLORS.textPrimary} />
                  </Bounce>
                  <View style={styles.mapCtrlDivider} />
                  <Bounce style={[styles.mapCtrlBtn, styles.mapCtrlBottom]} onPress={() => zoomBy('out')}>
                ...
                {(() => {
                  const sel = locations.find(l => l.location_id === selectedStation);
                  if (!sel) return null;
                  return (
                    <View style={styles.mapInfoCard}>
                      ...
                      <View style={{ flex: 1 }}>
                        <Text style={styles.mapInfoName} numberOfLines={1}>{sel.name}</Text>
                        <Text style={styles.mapInfoAddr} numberOfLines={1}>{sel.address}, {sel.city}</Text>
                      </View>
                      {sel.phone ? (
                        <Bounce style={styles.mapInfoCall} onPress={() => Linking.openURL(`tel:${sel.phone}`).catch(() => {})}>
            ...
            {locations.length === 0 ? (
              ...
              locations.map((loc, i) => (
                <Bounce
                  ...
                  scaleTo={0.98}
                  onPress={() => { handleMarkerPress(loc); setViewMode('map'); }}
                  style={[styles.locationCard, selectedStation === loc.location_id && styles.locationCardSelected]}
                >
                  <View style={[styles.locationIconWrap, selectedStation === loc.location_id && { backgroundColor: COLORS.secondary }]}>
                    <Ionicons name="storefront" size={17} color={selectedStation === loc.location_id ? COLORS.white : COLORS.primary} />
                  </View>
                  <View style={{ flex: 1 }}>
                  ...
                  <Ionicons name="navigate-circle-outline" size={20} color={COLORS.textMuted} />
                </Bounce>
              ))
            )}
          </Animated.View>
            ...
            {loading ? (
              ...
              <View style={styles.emptyTx}>
                ...
                <Text style={styles.emptyTxSub}>
                  {txFilter === 'credit' ? 'Top-ups will appear here'
                    : txFilter === 'debit' ? 'Trip payments will appear here'
                    : 'Your payment history will appear here'}
                </Text>
              </View>
            ) : (
              groupByDay(filteredTx).map((group) => (
                    ...
                    {group.items.map((t, idx) => {
                      ...
                      return (
                        <Bounce
                          ...
                          style={[styles.txRow, idx < group.items.length - 1 && styles.txRowDivider]}
                        >
                          <View style={[styles.txIconWrap, { backgroundColor: isCredit ? COLORS.secondaryLight : COLORS.dangerLight }]}>
                            <Ionicons name={isCredit ? 'arrow-down' : 'arrow-up'} size={18} color={isCredit ? COLORS.secondary : COLORS.danger} />
                          </View>
                          <View style={{ flex: 1 }}>
                            <Text style={styles.txTitle} numberOfLines={1}>{txTitle(t)}</Text>
                            <Text style={styles.txSub} numberOfLines={1}>{txSubtitle(t)}</Text>
                          </View>
                          <View style={{ alignItems: 'flex-end' }}>
                            <Text style={[styles.txAmount, { color: isCredit ? COLORS.secondary : COLORS.danger }]}>
                              {isCredit ? '+' : '-'}${parseFloat(t.amount).toFixed(2)}
                            </Text>
                            <Text style={styles.txTime}>{fmtTime(t.created_at)}</Text>
                          </View>
                        </Bounce>
                      );
                    })}
      ...
      <Modal visible={showCashier} transparent animationType="fade" onRequestClose={() => setShowCashier(false)}>
        <Pressable style={styles.modalOverlay} onPress={() => setShowCashier(false)}>
          <Animated.View
            style={[styles.cashierCard, {
              opacity: cashierAnim,
              transform: [
                { scale: cashierAnim.interpolate({ inputRange: [0, 1], outputRange: [0.85, 1] }) },
                { translateY: cashierAnim.interpolate({ inputRange: [0, 1], outputRange: [30, 0] }) },
              ],
            }]}
          >
            <Pressable>
              ...
              <View style={styles.cashierHeader}>
                <GradientFill id="cashierHead" colors={['#2563EB', '#7C3AED']} radius={0} />
                <View style={styles.cashierHeaderRow}>
                  <Text style={styles.cashierHeaderTitle}>Top Up at a Counter</Text>
                  <Bounce onPress={() => setShowCashier(false)} style={styles.cashierClose}>
                    <Ionicons name="close" size={18} color={COLORS.white} />
                  </Bounce>
                </View>
                <Text style={styles.cashierHeaderSub}>Show this code to the cashier — they’ll scan it to add funds instantly.</Text>
              </View>

              <View style={styles.cashierBody}>
                {/* glowing QR */}
                <View style={styles.qrShell}>
                  <Animated.View style={[styles.qrGlow, {
                    opacity: glow.interpolate({ inputRange: [0, 1], outputRange: [0.15, 0.5] }),
                    transform: [{ scale: glow.interpolate({ inputRange: [0, 1], outputRange: [0.95, 1.08] }) }],
                  }]} />
                  <View style={styles.qrBox}>
                    {qrValue ? (
                      <QRCode value={qrValue} size={172} color={COLORS.textPrimary} backgroundColor={COLORS.white} />
                    ) : (
                      <View style={styles.qrPlaceholder}><Ionicons name="qr-code-outline" size={70} color={COLORS.border} /></View>
                    )}
                  </View>
                </View>

                <View style={styles.cashierIdPill}>
                  <Ionicons name="finger-print" size={15} color={COLORS.primary} />
                  <Text style={styles.cashierIdText}>User ID: {userId ?? '—'}</Text>
                </View>

                <View style={styles.cashierInfo}>
                  <View style={styles.cashierInfoRow}>
                    <Ionicons name="person-outline" size={15} color={COLORS.textMuted} />
                    <Text style={styles.cashierInfoText} numberOfLines={1}>{userName}</Text>
                  </View>
                  {user?.email ? (
                    <View style={styles.cashierInfoRow}>
                      <Ionicons name="mail-outline" size={15} color={COLORS.textMuted} />
                      <Text style={styles.cashierInfoText} numberOfLines={1}>{user.email}</Text>
                    </View>
                  ) : null}
                  {user?.phone ? (
                    <View style={styles.cashierInfoRow}>
                ...
                <View style={styles.cashierHintRow}>
                  ...
                  <Text style={styles.cashierHint}>No scanner? The cashier can search by your ID, email, or phone.</Text>
                </View>
              </View>
            </Pressable>
          </Animated.View>
      ...
      {/* ══ Receipt Modal ══ */}
      <Modal visible={!!selectedTx} transparent animationType="none" onRequestClose={() => setSelectedTx(null)}>
        <Pressable style={styles.modalOverlayEnd} onPress={() => setSelectedTx(null)}>
          <Animated.View
            style={[styles.receiptCard, {
              paddingBottom: insets.bottom + 18,
              maxHeight: '92%',
              transform: [{ translateY: receiptAnim.interpolate({ inputRange: [0, 1], outputRange: [400, 0] }) }],
            }]}
          >
            <Pressable>
              {selectedTx && (() => {
                ...
                return (
                  <>
                    ...
                    <View style={styles.receiptHeader}>
                      <GradientFill
                        id="receiptHead"
                        colors={isCredit ? ['#10B981', '#059669'] : ['#F87171', '#DC2626']}
                        radius={22}
                      />
                      <View style={styles.receiptHeaderIcon}>
                        <Ionicons name={isCredit ? 'arrow-down' : 'arrow-up'} size={24} color={COLORS.white} />
                      </View>
                      <Text style={styles.receiptHeaderAmount}>
                        {isCredit ? '+' : '-'}${parseFloat(selectedTx.amount).toFixed(2)}
                      </Text>
                      <Text style={styles.receiptHeaderType}>{isCredit ? 'Wallet Top-Up' : 'Trip Payment'}</Text>
                    ...
                    <View style={styles.receiptDetails}>
                      <DetailRow icon="document-text-outline" label="Description" value={selectedTx.description} />
                      {selectedTx.location_name ? <DetailRow icon="storefront-outline" label="Location" value={selectedTx.location_name} /> : null}
                      {selectedTx.processed_by ? <DetailRow icon="person-circle-outline" label="Processed by" value={selectedTx.processed_by} /> : null}
                      {selectedTx.tx_ref ? <DetailRow icon="barcode-outline" label="Reference" value={selectedTx.tx_ref} mono /> : null}
                      <DetailRow icon="time-outline" label="Date & Time" value={fmtDateTime(selectedTx.created_at)} />
                      {selectedTx.transaction_id ? <DetailRow icon="receipt-outline" label="Transaction ID" value={`#${selectedTx.transaction_id}`} mono last /> : null}
                    </View>

                    <View style={styles.receiptBtnRow}>
                      <TouchableOpacity onPress={() => shareReceipt(selectedTx)} style={styles.shareBtn} activeOpacity={0.85}>
                        <Ionicons name="share-social-outline" size={18} color={COLORS.white} />
                        <Text style={styles.shareBtnText}>Share</Text>
                      </TouchableOpacity>
                      <TouchableOpacity onPress={() => printReceipt(selectedTx)} style={styles.printBtn} activeOpacity={0.85}>
                        <Ionicons name="print-outline" size={18} color={COLORS.primary} />
                        <Text style={styles.printBtnText}>Print PDF</Text>
                      </TouchableOpacity>
                    </View>
                    <TouchableOpacity onPress={() => setSelectedTx(null)} style={styles.receiptCloseBtn} activeOpacity={0.85}>
...
const styles = StyleSheet.create({
  ...
  iconBtn: {
    ...
    ...CARD_SHADOW, shadowOpacity: 0.06, elevation: 2,
  },
  ...
  card: {
    height: 210, borderRadius: 26, padding: 22,
    justifyContent: 'space-between',
    shadowColor: '#4F46E5', shadowOffset: { width: 0, height: 12 },
    shadowOpacity: 0.32, shadowRadius: 22, elevation: 10,
  },
  cardBg: { ...StyleSheet.absoluteFillObject, borderRadius: 26, overflow: 'hidden' },
  orb: { position: 'absolute', backgroundColor: 'rgba(255,255,255,0.07)' },
  ...
  chip: {
    ...
    backgroundColor: '#F6D38A', padding: 5, justifyContent: 'space-between',
  },
  chipLine: { height: 2.5, borderRadius: 2, backgroundColor: 'rgba(120,80,0,0.35)' },
  cardBrandText: { fontSize: 13, fontWeight: '800', color: 'rgba(255,255,255,0.9)', letterSpacing: 1.5 },
  frozenChip: {
  ...
  balanceLabel: { fontSize: 12, color: 'rgba(255,255,255,0.7)', fontWeight: '500', marginBottom: 2 },
  ...
  cardFootLabel: { fontSize: 9, color: 'rgba(255,255,255,0.55)', fontWeight: '700', letterSpacing: 1, marginBottom: 3 },
  cardFootValue: { fontSize: 14, color: COLORS.white, fontWeight: '700', maxWidth: SCREEN_W * 0.5 },
  ...
  statValue: { fontSize: 20, fontWeight: '900', color: COLORS.textPrimary, letterSpacing: -0.5 },
  ...
  cashierCtaBg: { ...StyleSheet.absoluteFillObject, borderRadius: 18, overflow: 'hidden' },
  cashierCtaInner: { flexDirection: 'row', alignItems: 'center', gap: 14, padding: 16 },
  cashierCtaIcon: {
  ...
  frozenBanner: {
    ...
    backgroundColor: COLORS.dangerLight, borderRadius: 18, padding: 14, marginBottom: 18,
    borderWidth: 1, borderColor: COLORS.dangerMid,
  },
  frozenIcon: { width: 38, height: 38, borderRadius: 12, backgroundColor: COLORS.white, alignItems: 'center', justifyContent: 'center' },
  frozenTitle: { fontSize: 14, fontWeight: '700', color: COLORS.dangerDark, marginBottom: 3 },
  ...
  noticePillText: { flex: 1, fontSize: 12, color: COLORS.textSecondary, lineHeight: 17 },

  /* View toggle */
  viewToggle: { flexDirection: 'row', backgroundColor: COLORS.surfaceAlt, borderRadius: 11, padding: 3, gap: 2 },
  toggleBtn: { flexDirection: 'row', alignItems: 'center', gap: 4, paddingHorizontal: 11, paddingVertical: 6, borderRadius: 8 },
  toggleBtnActive: { backgroundColor: COLORS.primary },
  toggleText: { fontSize: 12, fontWeight: '700', color: COLORS.textMuted },
  toggleTextActive: { color: COLORS.white },

  /* Map */
  mapContainer: { borderRadius: 20, overflow: 'hidden', marginBottom: 14, height: 290, position: 'relative', ...CARD_SHADOW, shadowOpacity: 0.12 },
  map: { flex: 1 },
  mapScrim: { position: 'absolute', top: 0, left: 0, right: 0, height: 60, backgroundColor: 'rgba(15,23,42,0.16)' },
  mapControls: {
    position: 'absolute', top: 12, right: 12, borderRadius: 14, backgroundColor: COLORS.white, overflow: 'hidden', ...CARD_SHADOW,
  },
  mapCtrlBtn: { width: 42, height: 42, alignItems: 'center', justifyContent: 'center' },
  mapCtrlTop: {},
  mapCtrlBottom: {},
  mapCtrlDivider: { height: 1, backgroundColor: COLORS.border, marginHorizontal: 6 },
  myLocBtn: { position: 'absolute', top: 110, right: 12, width: 42, height: 42, borderRadius: 14, backgroundColor: COLORS.white, alignItems: 'center', justifyContent: 'center', ...CARD_SHADOW },
  stationBadge: {
    position: 'absolute', top: 14, left: 12, flexDirection: 'row', alignItems: 'center', gap: 5,
    backgroundColor: 'rgba(37,99,235,0.95)', borderRadius: 20, paddingHorizontal: 11, paddingVertical: 6,
  },
  ...
  markerPinActive: { backgroundColor: COLORS.secondary, transform: [{ scale: 1.15 }] },
  markerTip: {
  ...
  mapInfoIcon: { width: 40, height: 40, borderRadius: 12, backgroundColor: COLORS.secondary, alignItems: 'center', justifyContent: 'center' },
  ...
  mapInfoCall: { width: 40, height: 40, borderRadius: 12, backgroundColor: COLORS.primary, alignItems: 'center', justifyContent: 'center' },
  ...
  locationCard: {
    ...
    borderWidth: 1.5, borderColor: 'transparent', ...CARD_SHADOW, shadowOpacity: 0.06, elevation: 2,
  },
  locationCardSelected: { borderColor: COLORS.secondary, backgroundColor: COLORS.secondaryLight },
  locationIconWrap: { width: 40, height: 40, borderRadius: 12, backgroundColor: COLORS.primaryLight, alignItems: 'center', justifyContent: 'center', flexShrink: 0 },
  ...
  locationPhoneLink: { color: COLORS.primary, fontWeight: '600' },
  ...
  emptyTxIcon: { width: 64, height: 64, borderRadius: 22, backgroundColor: COLORS.primaryLight, alignItems: 'center', justifyContent: 'center' },
  ...
  txGroupLabel: { fontSize: 12, fontWeight: '700', color: COLORS.textMuted, marginBottom: 8, marginLeft: 4, textTransform: 'uppercase', letterSpacing: 0.5 },
  txGroupCard: { backgroundColor: COLORS.white, borderRadius: 18, paddingHorizontal: 14, ...CARD_SHADOW, shadowOpacity: 0.07, elevation: 2 },
  ...
  txTime: { fontSize: 11, color: COLORS.textMuted, marginTop: 3, fontWeight: '500' },

  /* Modals */
  modalOverlay: { flex: 1, backgroundColor: 'rgba(15,23,42,0.6)', justifyContent: 'center', alignItems: 'center', padding: 24 },
  modalOverlayEnd: { flex: 1, backgroundColor: 'rgba(15,23,42,0.6)', justifyContent: 'flex-end' },

  /* Cashier modal */
  cashierCard: { width: '100%', maxWidth: 380, backgroundColor: COLORS.white, borderRadius: 26, overflow: 'hidden' },
  cashierHeader: { padding: 20, paddingBottom: 18, overflow: 'hidden' },
  cashierHeaderRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  cashierHeaderTitle: { fontSize: 18, fontWeight: '800', color: COLORS.white },
  cashierClose: { width: 32, height: 32, borderRadius: 11, backgroundColor: 'rgba(255,255,255,0.22)', alignItems: 'center', justifyContent: 'center' },
  cashierHeaderSub: { fontSize: 13, color: 'rgba(255,255,255,0.85)', lineHeight: 18, marginTop: 8 },
  cashierBody: { padding: 22, paddingTop: 18, alignItems: 'center' },
  qrShell: { alignItems: 'center', justifyContent: 'center', marginBottom: 18 },
  qrGlow: { position: 'absolute', width: 230, height: 230, borderRadius: 40, backgroundColor: COLORS.primary },
  qrBox: { padding: 16, borderRadius: 22, backgroundColor: COLORS.white, borderWidth: 1, borderColor: COLORS.border, ...CARD_SHADOW },
  qrPlaceholder: { width: 172, height: 172, alignItems: 'center', justifyContent: 'center' },
  cashierIdPill: { flexDirection: 'row', alignItems: 'center', gap: 6, backgroundColor: COLORS.primaryLight, borderRadius: 99, paddingHorizontal: 16, paddingVertical: 8, marginBottom: 16 },
  cashierIdText: { fontSize: 14, fontWeight: '800', color: COLORS.primary },
  cashierInfo: { alignSelf: 'stretch', backgroundColor: COLORS.background, borderRadius: 16, padding: 14, gap: 11, borderWidth: 1, borderColor: COLORS.border, marginBottom: 14 },
  cashierInfoRow: { flexDirection: 'row', alignItems: 'center', gap: 10 },
  cashierInfoText: { fontSize: 13, fontWeight: '600', color: COLORS.textPrimary, flex: 1 },
  cashierHintRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 6, alignSelf: 'stretch' },
  cashierHint: { fontSize: 12, color: COLORS.textMuted, lineHeight: 17, flex: 1 },

  /* Receipt sheet */
  receiptCard: { backgroundColor: COLORS.white, borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 22, paddingBottom: 34 },
  sheetHandle: { width: 44, height: 5, borderRadius: 3, backgroundColor: COLORS.border, alignSelf: 'center', marginBottom: 14 },
  receiptHeader: { borderRadius: 22, padding: 22, alignItems: 'center', overflow: 'hidden', marginBottom: 18 },
  receiptHeaderIcon: {
    width: 52, height: 52, borderRadius: 26, backgroundColor: 'rgba(255,255,255,0.22)',
    alignItems: 'center', justifyContent: 'center', marginBottom: 12,
  },
  receiptHeaderAmount: { fontSize: 40, fontWeight: '900', color: COLORS.white, letterSpacing: -0.8 },
  receiptHeaderType: { fontSize: 14, color: 'rgba(255,255,255,0.9)', fontWeight: '600', marginTop: 2 },
  receiptStatusPill: {
    flexDirection: 'row', alignItems: 'center', gap: 4, marginTop: 12,
    backgroundColor: 'rgba(255,255,255,0.22)', borderRadius: 99, paddingHorizontal: 12, paddingVertical: 5,
  },
  receiptStatusText: { fontSize: 12, fontWeight: '700', color: COLORS.white },
  receiptDetails: { backgroundColor: COLORS.background, borderRadius: 18, paddingHorizontal: 16, marginBottom: 18, borderWidth: 1, borderColor: COLORS.border },
  detailRow: { flexDirection: 'row', alignItems: 'flex-start', gap: 12, paddingVertical: 13, borderBottomWidth: 1, borderBottomColor: COLORS.border },
  detailIcon: { width: 32, height: 32, borderRadius: 10, backgroundColor: COLORS.primaryLight, alignItems: 'center', justifyContent: 'center' },
  detailLabel: { fontSize: 11, fontWeight: '600', color: COLORS.textMuted, marginBottom: 2, textTransform: 'uppercase', letterSpacing: 0.3 },
  detailValue: { fontSize: 14, fontWeight: '700', color: COLORS.textPrimary, lineHeight: 19 },
  detailValueMono: { fontFamily: Platform.OS === 'ios' ? 'Menlo' : 'monospace', fontWeight: '600', fontSize: 13 },
  receiptBtnRow: { flexDirection: 'row', gap: 12, marginBottom: 10 },
  receiptCloseBtn: { backgroundColor: COLORS.surfaceAlt, borderRadius: 16, paddingVertical: 14, alignItems: 'center', justifyContent: 'center' },
  receiptCloseText: { fontSize: 15, fontWeight: '700', color: COLORS.textSecondary },
  shareBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: COLORS.primary, borderRadius: 16, paddingVertical: 15 },
  shareBtnText: { fontSize: 15, fontWeight: '800', color: COLORS.white },
  printBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, backgroundColor: COLORS.primaryLight, borderWidth: 1.5, borderColor: COLORS.primaryMid, borderRadius: 16, paddingVertical: 15 },
  printBtnText: { fontSize: 15, fontWeight: '800', color: COLORS.primary },
});
